...
int fmt_stm_load_song(song_t *song, slurp_t *fp, unsigned int lflags)
{
	...
	uint8_t tmp[4];
	int npat, n;
	uint16_t para_sdata[MAX_SAMPLES];
	...
	song->title[20] = '\0';
	slurp_seek(fp, 32, SEEK_SET); // skip the tag and stuff
...
// stolen from
// https://github.com/Clownacy/clowncommon/blob/master/clowncommon.h#L167
// :)
#define SCHISM_DIV_CEILING(a,b) (((a) + (b - 1)) / (b))
...
static uint8_t calculate_tempo_scale(uint8_t speed, uint8_t tempo)
{
	...
	speed = MIN(15, speed);
	original_tempo = SCHISM_DIV_CEILING(tempo * 6, speed);
	for (int i = 15; i > 0; --i) {
		for (int j = 0; j < 16; ++j) {
		calculated_tempo = SCHISM_DIV_CEILING(st2_tempo_table[i - 1][j] * 6, i);
